// i want to make an tokenizer that tokenizes a string into number and have two methods encode and decode;
...
const tokensBinary = {
  ...
  '?': '00111111', '@': '01000000', '[': '01011011', '\\': '01011100',
  ']': '01011101', '^': '01011110', '_': '01011111', '`': '01100000',
...
const Tokenizer = {
  encode: function (str) {
    ...
    for (let i = 0; i < str.length; i++) {
      tokens.push(tokensBinary[`${str[i]}`]);
    }
    return tokens;
  },
  ...
  decode: function (tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      str += Object.keys(tokensBinary).find(key => tokensBinary[key] === tokens[i])
    }
    return str;
  },
};
...
const result = document.getElementById('output-display');

encodeButton.addEventListener('click',()=>{
    if(!userInput.value) alert("please enter some value");
    const encodedToken = Tokenizer.encode(userInput.value);
    result.innerHTML = encodedToken;
});

decodeButton.addEventListener('click',()=>{
    if(!userInput.value) alert("please enter some value");
    const decodeToken = Tokenizer.decode(userInput.value.split(','));
    result.innerText = decodeToken;
})